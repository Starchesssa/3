...
def build_ffmpeg(json_file, images_dir="FFMPEG/Images", output="output.mp4"):
    ...
    input_parts = [f"-i {audio_file}"]  # Add audio first
    filter_parts = []
    idx = 0
    ...
    images = [item for item in data.get("images", []) if "image" in item]

    # Sort images by layer to keep background < mid < fore
    sorted_items = sorted(images, key=lambda x: LAYER_ORDER.get(x["layer"], 99))
    ...
    for item in sorted_items:
        ...
        input_parts.append(f"-loop 1 -t {duration} -i {os.path.join(images_dir, image_file)}")
    ...
    for item in sorted_items:
        ...
        filter_parts.append(f"[{idx+1}:v]{filter_expr}[v{idx}]")  # +1 because 0 is audio
        idx += 1

    filter_complex = ";".join(filter_parts)
    cmd = f"ffmpeg {' '.join(input_parts)} -filter_complex \"{filter_complex}\" -map 0:a -pix_fmt yuv420p {output}"
    return cmd
