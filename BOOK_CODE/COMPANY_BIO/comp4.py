...
def split_into_sections(script_text):
    """
    Split into intro + numbered lessons.
    Returns a dict { "Intro": "...", "Lesson_1": "...", ... }
    """
    parts = re.split(r"(KEY LESSON: .+)", script_text)
    sections = {}
    lesson_count = 0
...
def main():
    ...
    selected_file = txt_files[0]  # pick first file
    ...
    for section_name, section_text in sections.items():
        output_filename = os.path.join(OUTPUT_DIR, f"{section_name}.wav")
        combined_script = TONE_INSTRUCTION + section_text
        print(f"[{api_key[:8]}...] Generating TTS for section: {section_name}")
