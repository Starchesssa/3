...
def generate_script(chapter_number, chapter_title, book_title, all_chapters, api_index):
    ...
    prompt = (
        ...
        "use really simple language, always use more simple words , avoid jargons , speak like you are explaining one who has no idea in the profesion and each sentence must end with a dot [.] so that we know each sentence ending.\n"
        "always start by making people clearly undertand the concept or lesson either by statistics or not then follow other after clearly conveying what the lesson means ,please make people undertand the concept as conveyed in the book"
...
def main():
    ...
    for i, chapter_title in enumerate(chapters, 1):
        ...
        try:
            script, api_index = generate_script(i, chapter_title, book_title, chapters, api_index)
