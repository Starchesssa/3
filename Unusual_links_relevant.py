...
def process_link(link, product, model, client, client_index, retries=0):
    ...
    try:
        ...
        print(f"🧾 Gemini Response for {link}: '{response_obj.text.strip()}'")
        ...
        print(f"❌ Error calling Gemini for {link} with model {model}: {e}")
        if retries < 1:
            print(f"🔁 Retrying... Attempt {retries + 2}")
            time.sleep(5)
...
def process_links():
    ...
    print(f"📁 Found {len(txt_files)} link files in '{LINKS_DIR}'.")

    for file_index, file_name in enumerate(txt_files):
        if qualified_files >= MAX_QUALIFIED_TXT:
            print(f"✅ Limit reached: {qualified_files} qualified files.")
            break
        ...
        print(f"🔎 Product Target: '{product}' with {len(links)} links")
        ...
        with ThreadPoolExecutor(max_workers=len(checked_links)) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    results[link] = result
                    print(f"🔎 Result: {link} => {result.upper()}")
                except Exception as e:
